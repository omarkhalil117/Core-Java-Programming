...
public class InMemoryWorldDao implements WorldDao {
        ...
        public static void main(String args[]) {
                ...
                // get the max populated city in each country and store it on result Map
                worldDao.countries.forEach((k, v) -> {
                        City maxPopulationCity = v.getCities().stream()
                                        .filter(e -> e != null)
                                        .max(Comparator.comparingDouble(City::getPopulation))
                                        .orElse(null);

                        result.put(k, maxPopulationCity);
                });

                // print the result and ignore nulls
                result.forEach((k, v) -> {
                        if (v != null)
                ...
                // Map to store cities for each continent
                // Map<String, List<City>> continentCities = new TreeMap<>();

                // // get cities of each continent and add it to continentCities Map
                // worldDao.countries.forEach((k, v) -> {
                ...
                // // Store most populated city at each continent
                // Map<String, City> res = new HashMap<>();

                // // get the max population of each continent
                // continentCities.forEach((k, v) -> {
                // String cont = k;
                // City maxPopulatedCity = v.stream()
                ...
                // City res = capitalCities
                // .stream().max(Comparator.comparingInt(City::getPopulation))
                // .orElse(null);

                // System.out.println("Name : " + res.getName() + " | Population : " +
                // res.getPopulation());

                // --------------------------------------------------------------------------------------------------

                // [3] Using Streams
                // List<City> capitalCities = worldDao.countries.values().stream()
                // .map(Country::getCapital) // Get capital city code
                // .flatMap(code -> worldDao.cities.values().stream()
                // .filter(city -> city.getId() == code))
                // .collect(Collectors.toList());

                // City res = capitalCities.stream()
                // .max(Comparator.comparingInt(City::getPopulation))
                // .orElse(null);
